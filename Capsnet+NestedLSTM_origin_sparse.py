...
y1 = np.load(data_source_path+'train_point_value_sparse.npy')
...
testX = np.load(data_source_path+'test_img_sparse.npy')
...
y_1 = np.load(data_source_path+'test_point_value_sparse.npy')
...
if __name__ == '__main__':
    if button == 'train':
        ...
        model.fit(x_train, y_train,
                ...
                callbacks=[ModelCheckpoint('./h5/capsnet+nlstm_origin_sparse_{}_{}.h5'.format(str(timesteps), str(pre_steps)), monitor='val_loss' , save_best_only=True, save_weights_only=False,verbose=1)])
        ...
        model = keras.models.load_model('./h5/capsnet+nlstm_origin_sparse_{}_{}.h5'.format(str(timesteps), str(pre_steps)), custom_objects={'CapsuleLayer': CapsuleLayer, 'NestedLSTM': NestedLSTM})
        model.summary()
        score = model.evaluate(x_test, y_test)
        print('Test score:', score)
        ...
        for i in range(pre_steps): #10
            ...
            img.convert('L').save('./test/capsnet+nlstm_origin_sparse_{}_{}_{}.png'.format(str(timesteps), str(pre_steps), str(i)))
        ...
        for i in range(pre_steps):
            ...
            img = Image.fromarray(array)
            img.convert('L').save('./predictions/capsnet+nlstm_origin_sparse_{}_{}_{}.png'.format(str(timesteps), str(pre_steps), str(i)))
