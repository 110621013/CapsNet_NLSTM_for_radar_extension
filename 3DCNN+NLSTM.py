...
if __name__ == '__main__':
    if button == 'train':
        ...
        model.fit(x_train, y_train,
                ...
                callbacks=[ModelCheckpoint('./h5/3dcnn+nlstm_{}_{}.h5'.format(str(timesteps), str(pre_steps)), monitor='val_loss' , save_best_only=True, save_weights_only=False,verbose=1)])
        ...
        model = keras.models.load_model('./h5/3dcnn+nlstm_{}_{}.h5'.format(str(timesteps), str(pre_steps)), custom_objects={'CapsuleLayer': CapsuleLayer, 'NestedLSTM': NestedLSTM})
        model.summary()
        score = model.evaluate(x_test, y_test)
        print('Test score:', score)
        ...
        for i in range(pre_steps): #10
            ...
            img.convert('L').save('./test/3dcnn+nlstm_{}_{}_{}.png'.format(str(timesteps), str(pre_steps), str(i)))
        ...
        for i in range(pre_steps):
            ...
            img = Image.fromarray(array)
            img.convert('L').save('./predictions/3dcnn+nlstm_{}_{}_{}.png'.format(str(timesteps), str(pre_steps), str(i)))
